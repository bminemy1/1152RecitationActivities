...
%problem 2
\begin{problem}
Verify that both $f(x)=1-(x^2+1)^2$ \dfn{and} $g(x) = 1 - (x^2-1)^2$ are solutions to the initial value problem
	\[
	\dd[y]{x} = -4x\sqrt{1-y}  \qquad  y(0) = 0.
	\]
...
Find a specific solution to the differential equation $\dd[y]{x} = x^{-2} \arctan(x)$ if $y(1)=5$.
	...
	Therefore
		\[
		\boxed{y(t) = - \frac{1}{x} \arctan(x) + \ln|x| - \frac{1}{2} \ln(1+x^2) + 5 + \frac{\pi}{4} + \frac{1}{2} \ln(2).}
		\]
	\end{freeResponse}
...
Find a specific solution to the initial value problem
	...
	Thus,
		\[
		\boxed{y(t) = -x^2\cos(x) + 2x\sin(x) + 2\cos(x) + 3.}
		\]
	\end{freeResponse}
...
Explain why the functions with the given graphs cannot be solutions of the differential equation $y' = e^x (y-1)^2$.
	...
	One property of $y(x)$ that we see on this graph is that $y(1) = 1$.  
	Also, in this graph we see that the slope of the tangent line at $x=1$ is positive, but 
		\[
		\eval{\dd[y]{x}}_{x=1} = e^1(1-1) = 0.
		\]
...
Students should eventually realize that differential equations are statements about the slope of the function at each point $(x,y)$.  
