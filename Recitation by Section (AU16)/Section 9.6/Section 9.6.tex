...
\title{Section 9.6: Alternating Series}  
...
\begin{itemize}
\item[A.] Explain what it means for a series $\displaystyle \sum_{k=1}^{\infty} a_k$ to converge absolutely.
\begin{freeResponse}
$\displaystyle \sum_{k=1}^{\infty} a_k$ to converge absolutely if the associated series $\displaystyle \sum_{k=1}^{\infty} |a_k|$ converges.
\end{freeResponse}
\item[B.]  Explain what it means for a series $\displaystyle \sum_{k=1}^{\infty} a_k$ to converge conditionally.
\begin{freeResponse}
$\displaystyle \sum_{k=1}^{\infty} a_k$ to converge conditionally if the associated series $\displaystyle \sum_{k=1}^{\infty} |a_k|$ diverges but the original series $\displaystyle \sum_{k=1}^{\infty} a_k$ converges.
\end{freeResponse}
\end{itemize}

\end{problem}

\begin{problem} Understanding what an alternating series is.
\begin{itemize}
\item[A.] Is the series $\displaystyle \sum_{k=1}^{\infty} \sin(k)$ alternating?  
\begin{freeResponse}
No; a series is alternating if and only if the terms \emph{strictly} alternate in sign!
\end{freeResponse}

\item[B.] Suppose $\{a_k\}$ is a sequence.  Is the series $\displaystyle \sum_{k=1}^{\infty} (-1)^ka_k$ alternating? If it is not, what assumption(s) would be needed on the terms in the sequence $\{a_k\}$ to ensure the series is alternating?
\begin{freeResponse}
No; there is no assumption here that $a_k>0$ for all $k$, so we are not guaranteed that $(-1)^k a_k$ is strictly alternating in sign!  If we assume that $a_k >0$ for all $k$, then the series  $\displaystyle \sum_{k=1}^{\infty} (-1)^k a_k$ would be alternating.
\end{freeResponse}
\end{itemize}
\end{problem}
